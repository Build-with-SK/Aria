...
def run_macro(config: dict):
    try:
        import src.macro.macro_data as mod
        cls = None
        for name in ("MacroDataFetcher", "MacroFetcher", "MacroData", "Macro"):
            cls = getattr(mod, name, None)
            if cls is not None:
                break
        if cls is None:
            for fname in ("get_macro_snapshot", "fetch_macro", "run_macro", "macro_snapshot"):
                fn = getattr(mod, fname, None)
                if fn is not None:
                    snapshot = fn(config)
                    logger.info(f"Macro snapshot: score={getattr(snapshot, 'macro_score', 'N/A')}")
                    return snapshot
            logger.warning("Macro snapshot failed: no recognised class/function in macro_data.py")
            return None
        fetcher = cls(config)
        for method in ("get_macro_snapshot", "fetch", "run", "get_snapshot", "snapshot"):
            ...
            if fn is not None:
                ...
                return snapshot
        logger.warning("Macro snapshot failed: fetcher found but no recognised method.")
        return None
    except Exception as e:
...
def run_sentiment(tickers: list, config: dict) -> dict:
    try:
        import src.sentiment.sentiment_analyzer as mod
        cls = None
        for name in ("SentimentAnalyzer", "Sentiment", "SentimentEngine",
                     "NewsAnalyzer", "SentimentScorer"):
            cls = getattr(mod, name, None)
            if cls is not None:
                break
        if cls is None:
            for fname in ("analyze_all", "run_sentiment", "get_sentiment", "analyze"):
                fn = getattr(mod, fname, None)
                if fn is not None:
                    results = fn(tickers, config)
                    logger.info(f"Sentiment complete for {len(results)} tickers.")
                    return results
            logger.warning("Sentiment failed: no recognised class/function in sentiment_analyzer.py")
            return {}
        analyzer = cls(config)
        for method in ("analyze_all", "run", "analyze", "get_all", "score_all"):
            fn = getattr(analyzer, method, None)
            if fn is not None:
                results = fn(tickers)
                logger.info(f"Sentiment analysis complete for {len(results)} tickers.")
                return results
        logger.warning("Sentiment failed: class found but no recognised method.")
        return {}
    except Exception as e:
